...
//	private static int ENTRY_SIZE = Integer.getInteger("gc-measure.entry-size", 104); // 64bit
	...
	private static char[] STRING_TEMPLATE = new char[16 * 1024];
	...
	private static void putRandom(int count) {
		...
		Object val = STRING_MODE ? 
				new String(new String(STRING_TEMPLATE).substring(0, 10 + random.nextInt(1 << random.nextInt(12)))) 
