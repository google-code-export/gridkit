...
public class ExtendConnection {
	...
	/**
	 * @param config - cache configuration file
	 */
	public ExtendConnection(XmlElement config) {
		LOGGER.info("New Extend connection #" + connectionId + " is going to be created");
		cacheFactory = initPrivateCacheFactory(config);
	}
	...
	private DefaultConfigurableCacheFactory initPrivateCacheFactory(String configFile) {
		...
		return initPrivateCacheFactory(xml);
	}

	private DefaultConfigurableCacheFactory initPrivateCacheFactory(XmlElement xml) {
