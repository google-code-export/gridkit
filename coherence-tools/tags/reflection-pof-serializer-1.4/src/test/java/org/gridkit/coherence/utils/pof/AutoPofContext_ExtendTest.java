...
public class AutoPofContext_ExtendTest extends AutoPofContext_FunctionalTest {
	...
	public static void init_storage_node() throws SecurityException, IllegalArgumentException, NoSuchFieldException, IllegalAccessException {
		...
		isolate1 = new Isolate("Remote1", "org.gridkit", "com.tangosol");
		isolate1.start();
		isolate1.submit(NodeActions.Start.class, "auto-pof-cache-config-extend-server.xml");
		isolate1.submit(NodeActions.GetService.class, "AUTO_POF_SERVICE");
		isolate1.submit(NodeActions.GetService.class, "TcpProxyService");

		System.setProperty("extend.port", "9100");

		isolate2 = new Isolate("Remote2", "org.gridkit", "com.tangosol");
		isolate2.start();
		isolate2.submit(NodeActions.Start.class, "auto-pof-cache-config-extend-server.xml");
		isolate2.submit(NodeActions.GetService.class, "AUTO_POF_SERVICE");
		isolate2.submit(NodeActions.GetService.class, "TcpProxyService");
	...
	public static void shutdown_stoarge_node() {
		isolate1.submit(NodeActions.Stop.class);
		isolate1.stop();
		isolate2.submit(NodeActions.Stop.class);
		isolate2.stop();
