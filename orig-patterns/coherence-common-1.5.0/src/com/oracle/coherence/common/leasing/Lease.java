...
public class Lease implements ExternalizableLite, PortableObject {
	...
	/**
	 * <p>Returns the time (since the epoc in milliseconds) since
	 * the {@link Lease} was initially acquired.</p>
	 */
	...
	/**
	 * <p>Returns the time (since the epoc in milliseconds) since
	 * the {@link Lease} was last updated.</p>
	 */
	...
	/**
	 * <p>Returns the duration (in milliseconds) that the {@link Lease}
	 * is valid (after the last update time).</p>
	 */
	...
	/**
	 * <p>Returns if the {@link Lease} has been issued indefinitely
	 * and thus never expires.</p>
	 */
	...
	/**
	 * <p>Returns if will {@link Lease} is valid at the
	 * specified time.</p>
	 * 
	...
	public boolean isValidAt(long time) {
		return isIndefinite() || 
			   (!isCanceled() && 
			    !isSuspended() &&
			    lastUpdateTime + duration >= time);
	}
	
	
	/**
	 * <p>Attempts to extend the currently owned {@link Lease} by a specified
	 * number of milliseconds from the current point in time.</p>
	 * 
	 * @param duration
	 * 
	...
	public boolean extend(long duration) {
		if (isIndefinite()) {
			...
			if (Logger.isEnabled(Logger.WARN)) Logger.log(Logger.WARN, "Attempted to extend %s by a negative duration %d. Ignoring request", this, duration);
			return false;
		}
	}
