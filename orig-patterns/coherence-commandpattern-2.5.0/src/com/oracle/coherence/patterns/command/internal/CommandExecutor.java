...
/**
 * <p>An {@link CommandExecutor} is responsible for coordinating and
 * executing {@link Command}s (represented as {@link CommandExecutionRequest}s) 
 * for a single a {@link Context}.</p>
 * 
 * <p>Included in this responsibility is the recovery and potential 
 * re-execution of {@link CommandExecutionRequest}s
 * (in-the-originally-submitted-order).  Such recovery may be due
 * to Coherence load-balancing or recovering cache partitions.</p>
 * 
 * <p>Internally {@link CommandExecutor}s operate as a simple
 * finite-state-machine (see {@link State} for details on states).</p>
 * 
 * <p>{@link CommandExecutor}s state transitions are typically controlled 
 * via 'events' delivered a {@link ContextBackingMapListener} or through the
 * {@link SubmitCommandExecutionRequestProcessor}.</p>
 * 
 * <p>All {@link CommandExecutor}s are monitorable through JMX.  They are 
 * automatically registered and de-registered when they created and stopped
 * (respectively).</p>
 * 
 * <p>Use a {@link ContextsManager} register and manage {@link Context}s.</p>
 *  
...
public class CommandExecutor implements CommandExecutorMBean {
	...
	/**
	 * <p>The {@link Identifier} of the {@link Context} for which this {@link CommandExecutor} is 
	 * coordinating {@link CommandExecutionRequest} execution.</p>
	 *   
	 * <p>{@link Context} {@link Identifier}s are unique across a cluster.</p>
	 */
	private Identifier contextIdentifier;

	
	/**
	 * <p>The {@link ContextConfiguration} for the {@link Context} 
	 * being managed by this {@link CommandExecutor}.</p>
	 */
	private ContextConfiguration contextConfiguration;

	
	/**
	 * <p>The {@link BackingMapManagerContext} to which this {@link CommandExecutor}
	 * is associated.  This is used to help locate the underlying {@link PartitionedService}
	 * on which the {@link CommandExecutor} depends.</p>
	 */
	private BackingMapManagerContext backingMapManagerContext;
	
	
	/**
	 * <p>The set of possible {@link State}s for a {@link CommandExecutor}.</p> 
	 */
	public static enum State {

		/**
		 * <p>The <code>New</code> state indicates that a {@link CommandExecutor}
		...
		/**
		 * <p>The <code>Starting</code> state indicates that a {@link CommandExecutor}
		...
		/**
		 * <p><code>Waiting</code> indicates that a {@link CommandExecutor} has 
		 * been successfully started but is currently waiting for 
		 * {@link CommandExecutionRequest}s to arrive.</p>
		 */
		Waiting,
		
		/**
		 * <p><code>Scheduled</code> indicates that a {@link CommandExecutor} has 
		 * been been scheduled to execute {@link CommandExecutionRequest}s,
		 * but has yet to commence execution.</p>
		 */
		Scheduled,
		
		/**
		 * <p><code>Executing</code> indicates that a {@link CommandExecutor} 
		 * is in the process of executing {@link CommandExecutionRequest}s.</p>
		 */
		Executing,
		
		/**
		 * <p><code>Stopped</code> indicates that a {@link CommandExecutor} is
	...
	/**
	 * <p>The current {@link State} of the {@link CommandExecutor}.</p>
	 */
	private volatile State state;
	
	
	/**
	 * <p>A {@link SequenceGenerator} that may be used to generate ticket issuer ids</p>
	 */
	private SequenceGenerator sequenceGenerator;
	
	
	/**
	 * <p>The current ticket issuer id that this {@link CommandExecutor}
	 * is using for issuing {@link Ticket}s.  Over time this value may change
	 * as it's not dependent on an individual instance of a {@link CommandExecutor}.</p>
	 */
	private long ticketIssuerId;
	
	
	/**
	 * <p>The ordered set of {@link TicketBook}s representing the {@link Ticket}s
	...
	/**
	 * <p>The in-order collection of {@link Ticket}s for {@link CommandExecutionRequest}s 
	 * that the {@link CommandExecutor} has accepted and are currently pending execution.</p>
	 */
	...
	/**
	 * <p>The in-order collection of {@link Ticket}s for {@link CommandExecutionRequest}s
	 * that are have high-priority for execution by the {@link CommandExecutor}.</p>
	 */
	private TicketBook priorityTicketBook;
	
	
	/**
	 * <p>The version of the {@link Context} (ie: {@link ContextWrapper}) this
	 * {@link CommandExecutor} is managing.</p>
	 */
	private long contextVersion;
	
	
	/**
	 * <p>The cluster-wide uniquely generated JMX bean name for
	...
	/**
	 * <p>The local number of {@link CommandExecutionRequest}s that
	 * have been submitted for execution by the {@link CommandExecutor}.</p>
	 */
	...
	/**
	 * <p>The local number of {@link CommandExecutionRequest}s that 
	 * have been executed by the {@link CommandExecutor}.</p>
	 */
	private long localCommandsExecuted;
	
	
	/**
	 * <p>The time (in milliseconds) the most recently locally executed  
	 * {@link CommandExecutionRequest} took to execute.</p>
	 */
	private long localLastCommandExecutionDuration;
	
	
	/**
	 * <p>The total local time taken (in milliseconds) for the {@link CommandExecutionRequest}s
	 * executed thus far.</p>
	 */
	...
	/**
	 * <p>The minimum time (in milliseconds) that a {@link CommandExecutionRequest}
	 * has taken to execute (locally).</p>
	 */
	private long localMinimumCommandExecutionDuration;
	
	
	/**
	 * <p>The maximum time (in milliseconds) that a {@link CommandExecutionRequest}
	 * has taken to execute (locally).</p>
	 */
	private long localMaximumCommandExecutionDuration;
	
	/**
	 * <p>The total time it takes to execute a command including the time to 
	 * retriever the command.</p>
	 */
	private long localCommandExecutionServiceDuration;
	

	/**
	 * <p>The total of {@link CommandExecutionRequest}s that have been executed.</p>
	 */
	private long totalCommandsExecuted;
	
	
	/**
	 * <p>The total time taken (in milliseconds) for the {@link CommandExecutionRequest}s
	 * executed thus far.</p>
	 */
	...
	/**
	 * <p>The total time (in milliseconds) for the {@link CommandExecutionRequest}s
	 * to commence execution.</p>
	 */
	private long totalCommandExecutionWaitingDuration;
	
	
	/**
	 * <p>The maximum number of {@link CommandExecutionRequest}s to execute in a single batch.</p>
	 */
	private int maximumBatchSize;
	

	/**
	 * <p>The amount of time we should delay performing some task 
	 * when ownership of the {@link Context} (and {@link CommandExecutor}) is in doubt
	 * due to a partition being (potentially) moved.</p>
	 */
	 ...
	 * 
	 * <p><b>NOTE:</b> To construct a {@link CommandExecutor} for your application,
	 * you should use a {@link ContextsManager}.</p>
	 *  
	...
	/**
	 * <p>Returns the {@link Identifier} of the {@link Context} for which 
	 * this {@link CommandExecutor} is coordinating {@link CommandExecutionRequest} execution.</p>
	 * 
	 * <p>{@link Context} {@link Identifier}s are unique across a cluster.</p>
	 */
	...
	/**
	 * <p>Sets the {@link State} of the {@link CommandExecutor}.</p>
	 * 
	 * @param state
	 */
	public void setState(State state) {
		if (this.state == State.Stopped && state != State.Stopped) {
			Logger.log(Logger.ERROR, "Attempting to set the CommandExecutor %s state to %s when it's already Stopped", contextIdentifier, this.state);
		} else {
			this.state = state;
		}
	}
	...
	/**
	 * <p>Returns the cluster-wide uniquely generated JMX bean name for
	 * the {@link CommandExecutor}.</p>
	 */
	...
	/**
	 * <p>Returns if the {@link Context} is still owned by the Cluster {@link Member} (JVM)
	 * in which the {@link Context} is running.</p>
	 * 
	 * <p>This method is used to help us discover if a partition is in the process of moving.</p>
	 */
	...
	/**
	 * <p>Starts up, initializes, recovers and schedules the {@link CommandExecutor}
	 * to start processing {@link CommandExecutionRequest}s.</p>
	 */
	@SuppressWarnings("unchecked")
	public void start() {
		if (getState() == State.New) {
			if (Logger.isEnabled(Logger.DEBUG)) Logger.log(Logger.DEBUG, "Starting CommandExecutor for %s", contextIdentifier);
						
			//claim ownership of the Context
			NamedCache contextsCache = CacheFactory.getCache(ContextWrapper.CACHENAME);			
			ContextWrapper contextWrapper = (ContextWrapper)contextsCache.invoke(contextIdentifier, new ClaimContextProcessor(CacheFactory.getCluster().getLocalMember().getUid()));
			if (contextWrapper == null) {
				if (Logger.isEnabled(Logger.DEBUG)) Logger.log(Logger.DEBUG, "CommandExecutor for %s can not be started.  The Context has been moved (or deleted) since the CommandExecutor was established and scheduled to start.  Now stopped.", contextIdentifier);
				stop();
				...
				if (Logger.isEnabled(Logger.DEBUG)) Logger.log(Logger.DEBUG, "CommandExecutor for %s has been configured as %s", contextIdentifier, contextConfiguration);
				
				//recover any previously accepted CommandExecutionRequests (but have not been/completed execution)
				if (Logger.isEnabled(Logger.DEBUG)) Logger.log(Logger.DEBUG, "Recovering unexecuted commands for CommandExecutor %s ", contextIdentifier);
				...
				recoveredTicketBookSet.addAll(
					(LinkedList<TicketBook>)CacheFactory.getCache(CommandExecutionRequest.getCacheName(ManagementStrategy.DISTRIBUTED)).aggregate(
						filter,
						new TicketAggregator("getTicket")));

				//aggregate all of the COLOCATED commands to recover
				recoveredTicketBookSet.addAll(
						(LinkedList<TicketBook>)CacheFactory.getCache(CommandExecutionRequest.getCacheName(ManagementStrategy.COLOCATED)).aggregate(
							new KeyAssociatedFilter(filter, getContextIdentifier()),
							new TicketAggregator("getTicket")));
				...
				synchronized(this) {
					...
					if (recoveredTicketBooks.size() > 0) {
						if (Logger.isEnabled(Logger.DEBUG)) Logger.log(Logger.DEBUG, "Recovered %d Tickets for CommandExecutor %s. Books are %s", getTotalCommandsPendingExecution() - ticketBook.size(), contextIdentifier, recoveredTicketBooks);
						
					} else {
						if (Logger.isEnabled(Logger.DEBUG)) Logger.log(Logger.DEBUG, "No commands to recover for CommandExecutor %s", contextIdentifier);
					}
				}
				...
				if (registry != null) {
					if (Logger.isEnabled(Logger.DEBUG)) Logger.log(Logger.DEBUG, "Registering JMX management extensions for CommandExecutor %s", contextIdentifier);
					setMBeanName(registry.ensureGlobalName(String.format("type=CommandExecutor,id=%s", contextIdentifier)));
				...
				if (recoveredTicketBooks.isEmpty() && ticketBook.isEmpty()) {
					if (Logger.isEnabled(Logger.DEBUG)) Logger.log(Logger.DEBUG, "No commands to execute for CommandExecutor %s.  (waiting for commands to be submitted)", contextIdentifier);
					
					setState(State.Waiting);
				} else {
					if (Logger.isEnabled(Logger.DEBUG)) Logger.log(Logger.DEBUG, "Scheduling CommandExecutor %s to execute commands", contextIdentifier);
				...
				if (Logger.isEnabled(Logger.DEBUG)) Logger.log(Logger.DEBUG, "Started CommandExecutor for %s", contextIdentifier);
			} 
			
		} else {
			if (Logger.isEnabled(Logger.DEBUG)) Logger.log(Logger.DEBUG, "Attempted to start CommandExecutor for %s, but it's already Started (current state %s)", contextIdentifier, getState());
		}
	}
	
	
	/**
	 * <p>Immediately stops the {@link CommandExecutor} from executing 
	 * any further {@link CommandExecutionRequest}s.</p>
	 */
	public void stop() {
	...
	/**
	 * <p>Accepts the specified {@link CommandExecutionRequest} for execution by this {@link CommandExecutor}.</p>
	 * 
	 * <p>As part of this process, the {@link CommandExecutionRequest} is issued with a {@link Ticket}, has
	 ...
	 * the {@link CommandExecutionRequest} for execution.</p>
	 * 
	 * <p>If the Member in which this {@link CommandExecutor} is running no longer owns the {@link Context}
	...
	public synchronized SubmissionOutcome acceptCommandExecutionRequest(final CommandExecutionRequest commandExecutionRequest) {
		...
		if (getState() == State.Stopped) {
			if (Logger.isEnabled(Logger.DEBUG)) Logger.log(Logger.DEBUG, "CommandExecutor for %s can't accept %s as it has been Stopped", contextIdentifier, commandExecutionRequest);
			...
			Ticket ticket = (commandExecutionRequest.getCommand() instanceof PriorityCommand ? priorityTicketBook : ticketBook).extend();
			...
			CommandExecutionRequest.Key key = new CommandExecutionRequest.Key(contextIdentifier, ticket, managementStrategy);
			...
			if (managementStrategy == ManagementStrategy.COLOCATED) {
				...
				DistributedCacheService distributedCacheService = (DistributedCacheService)CacheFactory.getService("DistributedCacheForCommandPattern");
				...
				backingMapManager.getBackingMap(cacheName).put(
					backingMapManager.getContext().getKeyToInternalConverter().convert(key), 
			...
			if (Logger.isEnabled(Logger.DEBUG)) Logger.log(Logger.DEBUG, "CommandExecutor for %s can't accept %s as Context ownership is currently in doubt", contextIdentifier, commandExecutionRequest);
	...
	/**
	 * <p>The method is used to asynchronously execute queued {@link CommandExecutionRequest}s.</p>
	 */
	@SuppressWarnings("unchecked")
	public void execute() {	
		try {
			...
			if (getState() == State.Scheduled) {
				...
				if (isContextOwned()) {
					...
					if (getState() == State.Executing) {
						...
						if (contextWrapper.getContextVersion() == contextVersion) {
							...
							while (isContextOwned() &&
								   getState() == State.Executing &&
								   ticketsToExecute.hasNext()) {
								...
								CommandExecutionRequest.Key commandExecutionRequestKey = new CommandExecutionRequest.Key(contextIdentifier, ticket, managementStrategy);

								long startLocalServiceExecuteTime = CacheFactory.getSafeTimeMillis();
								
								//get the next CommandExecutionRequest to execute
								NamedCache commandExecutionRequestsCache = CacheFactory.getCache(CommandExecutionRequest.getCacheName(managementStrategy));
								CommandExecutionRequest commandExecutionRequest = (CommandExecutionRequest)commandExecutionRequestsCache.invoke(commandExecutionRequestKey, new StartCommandProcessor());
								...
								if (commandExecutionRequest == null) {
									...
									if (Logger.isEnabled(Logger.QUIET)) Logger.log(Logger.QUIET, "Skipping %s for CommandExecutor %s as it was canceled", commandExecutionRequest, contextIdentifier);
									...
									try {
										if (Logger.isEnabled(Logger.QUIET)) Logger.log(Logger.QUIET, "Executing %s for CommandExecutor %s", commandExecutionRequest, contextIdentifier);
										
										//execute the command
										commandExecutionRequest.getCommand().execute(environment);
																				
									} catch (RuntimeException runtimeException) {
										Logger.log(Logger.ERROR, "Failed to execute %s with CommandExecutor %s", commandExecutionRequestKey, contextIdentifier);
										...
										localLastCommandExecutionDuration = Math.max(0, endCommandExecutionTime - startCommandExecutionTime);
										localCommandExecutionServiceDuration += endCommandExecutionTime - startLocalServiceExecuteTime;
										long localLastCommandExecutionWaitingDuration = Math.max(0, startCommandExecutionTime - commandExecutionRequest.getInstantQueued());
										localCommandExecutionDuration += localLastCommandExecutionDuration;
										localMaximumCommandExecutionDuration = Math.max(localMaximumCommandExecutionDuration, localLastCommandExecutionDuration);
										localMinimumCommandExecutionDuration = Math.min(localMinimumCommandExecutionDuration, localLastCommandExecutionDuration);
										...
										if ((Boolean)contextWrapperCache.invoke(contextIdentifier, new UpdateContextProcessor(contextVersion, 
																															  environment.isContextUpdated() ? environment.getContext() : null, 
																															  ticket, 
											...
											if (Logger.isEnabled(Logger.DEBUG)) Logger.log(Logger.DEBUG, "Failed to update ContextWrapper for %s as the CommandExecutor no longer owns the Context.  Detected that the context version number has changed during UpdateContextProcessor.", contextIdentifier);						
											stop();																						
										}
									} //try
								...
								synchronized(this) {
									...
									if (executionTicketBook.isEmpty()) {
										if (Logger.isEnabled(Logger.QUIET)) Logger.log(Logger.QUIET, "Completed %s for ContextWrapper for %s", executionTicketBook, contextIdentifier);						
									}
								}							
							} //while
							...
							synchronized(this) {
								if (isContextOwned()) {
									if (getState() == State.Executing) {
										...
										if (ticketBook.isEmpty() && recoveredTicketBooks.isEmpty() && priorityTicketBook.isEmpty()) {
											setState(State.Waiting);
										...
										if (Logger.isEnabled(Logger.DEBUG)) Logger.log(Logger.DEBUG, "CommandExecutor for %s has been Stopped during execution.", contextIdentifier, getState());						
									}
									
								} else {
									if (Logger.isEnabled(Logger.DEBUG)) Logger.log(Logger.DEBUG, "Ownership for CommandExecutor %s is currently in doubt (after execution).  Delaying execution for %d seconds", contextIdentifier, OWNERSHIP_DOUBT_DELAY_SECONDS);
							...
							if (Logger.isEnabled(Logger.DEBUG)) Logger.log(Logger.DEBUG, "CommandExecutor for %s no longer owns the Context.  Detected that the context version number has changed.", contextIdentifier);						
							stop();
						}
						
					} else {
						if (Logger.isEnabled(Logger.DEBUG)) Logger.log(Logger.DEBUG, "CommandExecutor for %s was scheduled to execute, but no longer needs to do so.  (current state is %s, %d pending commands)", contextIdentifier, getState(), getTotalCommandsPendingExecution());		
					}
					
				} else {
					if (Logger.isEnabled(Logger.DEBUG)) Logger.log(Logger.DEBUG, "Ownership for CommandExecutor %s is currently in doubt.  Delaying execution for %d seconds", contextIdentifier, OWNERSHIP_DOUBT_DELAY_SECONDS);
				...
				if (Logger.isEnabled(Logger.DEBUG)) Logger.log(Logger.DEBUG, "CommandExecutor for %s was scheduled to execute, but it is no longer executable (current state is %s)", contextIdentifier, getState());		
			}

		} catch (RuntimeException runtimeException) {
			if (Logger.isEnabled(Logger.ERROR)) Logger.log(Logger.ERROR, "CommandExecutor for %s failed horribly...", contextIdentifier);		
	...
	/**
	 * <p>An internal implementation of an {@link ExecutionEnvironment}.</p>
	 */
	@SuppressWarnings("unchecked")
	static class Environment implements ExecutionEnvironment {
		...
		public void setContext(Context context) {
			if (context == null) { 
				throw new IllegalStateException(String.format("Attempted to set the Context known as %s to null for Command with %s", contextIdentifier, ticket));  
			} else {
		...
		public void saveCheckpoint(Object checkpoint) {
			...
			CommandExecutionRequest.Key commandExecutionRequestKey = new CommandExecutionRequest.Key(contextIdentifier, ticket, managementStrategy);
			CacheFactory.getCache(CommandExecutionRequest.getCacheName(managementStrategy)).invoke(
				commandExecutionRequestKey,
				new UpdaterProcessor("setCheckpoint", checkpoint)
			);
		}
