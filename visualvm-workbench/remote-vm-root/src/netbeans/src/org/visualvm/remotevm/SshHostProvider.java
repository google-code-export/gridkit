...
public class SshHostProvider {
    ...
    public SshHost createHost(final SshHostProperties hostDescriptor, final boolean createOnly, final boolean interactive) {
        try {
            ...
            try {
                pHandle = ProgressHandleFactory.createHandle(NbBundle.getMessage(SshHostProvider.class, "LBL_Searching_for_host") + hostName); // NOI18N
                ...
                try {
                    ...
                    if (interactive) {
                        DialogDisplayer.getDefault().notifyLater(new NotifyDescriptor.
                                Message(NbBundle.getMessage(SshHostProvider.class,
            ...
            if (inetAddress != null) {
                ...
                if (knownHost != null) {
                    ...
                    try {
                        newHost = new SshHost(hostName, storage);
                    } catch (Exception e) {
                    ...
                    if (newHost != null) {
                        ...
                        Set<SshHost> remoteHostsSet = remoteHosts.getDataSources(SshHost.class);
                        if (!createOnly && remoteHostsSet.contains(newHost)) {
                            storage.deleteCustomPropertiesStorage();
                            Iterator<SshHost> existingHosts = remoteHostsSet.iterator();
    ...
    public Host getHostByAddress(InetAddress inetAddress) {
        try {
            ...
            LOGGER.throwing(SshHostProvider.class.getName(), "getHostByAddress", ex);    // NOI18N
            return null;
        } finally {
            unlockHosts();
        }
    ...
    private SshHost getHostByAddressImpl(InetAddress inetAddress) {
        Set<SshHost> knownHosts = DataSourceRepository.sharedInstance().getDataSources(SshHost.class);
    ...
    private void initPersistedHosts() {
        if (HostsSupport.storageDirectoryExists()) {
            ...
            for (File file : files) {
                ...
                try {
                    persistedHost = new SshHost(hostName, storage);
                } catch (Exception e) {
                    LOGGER.throwing(SshHostProvider.class.getName(), "initPersistedHosts", e);    // NOI18N
    ...
    private static void notifyUnresolvedHosts(final Set<File> unresolvedHostsF, final Set<String> unresolvedHostsS) {
        RequestProcessor.getDefault().post(new Runnable() {
            public void run() {
                ...
                messagePanel.add(new JLabel(NbBundle.getMessage(SshHostProvider.class, "MSG_Unresolved_Hosts")), BorderLayout.NORTH); // NOI18N
                ...
                NotifyDescriptor dd = new NotifyDescriptor(
                        messagePanel, NbBundle.getMessage(SshHostProvider.class, "Title_Unresolved_Hosts"), // NOI18N
