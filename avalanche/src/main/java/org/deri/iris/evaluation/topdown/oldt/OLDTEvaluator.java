...
public class OLDTEvaluator implements ITopDownEvaluator {
	...
	class Node {
		...
		private void addChildNodesByEvaluatingBuiltins()
			...
			try {
				if (!hasDeepVariables(builtinTuple)) {
					// all variables should be shallow for direct buildin evaluation
		...
		private boolean hasDeepVariables(ITuple tuple) {
			int shallowVars = 0;
			...
			if (shallowVars != tuple.getVariables().size()) {
				return true;
			}
			else {
				return false;
