...
public class SparseRange 
    implements Range, ExternalizableLite, PortableObject, Iterable<Long>
    ...
    */
    public long size() 
        {
        long size = 0;
        for(ContiguousRange range : m_oSubRanges) size = size + range.size();
        return size;
        }
    ...
    */
    ...
    /**
    * Test whether a range is adjacent to any of the subranges.
    */
    ...
    private boolean testAdjacency(Range other) 
        ...
        for (ContiguousRange range : m_oSubRanges) 
            {
            if (other.isAdjacent(range)) return true;
            }
            return false;
        } 
    ...
    */
    private boolean testIntersection(Range other)
        ...
        if (other instanceof ContiguousRange)
            {
            return (this.contains(other.getFrom()) || this.contains(other.getTo()) ||
                    other.contains(this.getFrom()) || other.contains(this.getTo()) ) ;
            } 
        else
            {
            if (other instanceof SparseRange)
                ...
                for(ContiguousRange range : m_oSubRanges) 
                    {
                    if (other.intersects(range)) return true;
                    }
                return false;
                } 
            else 
                {
                return false; //TODO: Maybe should throw exception instead.
                }
            }
        }
        ...
        if (other.isEmpty()) 
            ...
            //merge the two sets of sub ranges
                    ...
                    while (leftSubRanges.hasNext() || rightSubRanges.hasNext()) 
                        {
                        if (left == null && leftSubRanges.hasNext()) left = leftSubRanges.next();
                        if (right == null && rightSubRanges.hasNext()) right = rightSubRanges.next();
    ...
    private boolean iterateForEquality(Range other)
        ...
        if (other instanceof SparseRange)
            ...
            while (thisSubRanges.hasNext() && otherSubRanges.hasNext()) 
                {
                if (!thisSubRanges.next().equals(otherSubRanges.next())) return false;
                }
    ...
    */
    class RangeIterator 
        ...
        */
        public Long next() 
            {
            if (!iterator.hasNext()) iterator = this.ranges.next().iterator();
