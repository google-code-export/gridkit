	...
	<xsl:template match="/">
		...
		<xsl:for-each select="$classes">
			...
			<xsl:result-document href="{$outputFile}">
				...
				<xsl:value-of select="'import com.griddynamics.skeleton.attribute.AttributeKey;&#xA;&#xA;'"/>
				<xsl:value-of select="fn:concat('public class ', af:lastName($class), ' { &#xA;')"/>

				<xsl:for-each select="$attributes[af:fullClass(name) = $currentClass]">
					<xsl:variable name="genericClass" select="fn:concat('AttributeKey&lt;' , type, '&gt;')"/>
					<xsl:value-of select="fn:concat('public static final ', $genericClass, ' ', af:lastName(name), ' = new ', $genericClass, '((short)', @id, ',', type, '.class ,&quot;', description , '&quot;);')"/>
				</xsl:for-each>
